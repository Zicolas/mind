...
class Creature:
    ...
    def update(self, others):
        # Neighbor stress
        neighbors = sum(1 for c in others if c is not self and abs(c.x - self.x) <= 1 and abs(c.y - self.y) <= 1)
...
with col1:
    ...
    if st.button("💾 Save State"):
        st.session_state.saved = [(c.x, c.y, c.energy, c.stress, c.name) for c in st.session_state.creatures]
...
if st.session_state.running:
    for _ in range(200):
        ...
        # Show stats
        stat_lines = [
            f"{c.name}: Energy={c.energy:.2f}, Stress={c.stress:.2f}, {'🔥 Disinhibited' if c.disinhibited else ''}"
            for c in st.session_state.creatures[:10]  # show first 10 only
        ]
