...
class Creature:
    ...
    def update(self, creatures, energy_sources):
        # Update parameters from sim settings in case changed
        self.habituation_rate = st.session_state.sim_params['habituation_rate']
        self.inhibition = st.session_state.sim_params['inhibition']
        ...
        # Stress spreads slightly from neighbors (simple contagion)
        ...
        if neighbors:
            ...
            self.stress += (avg_neighbor_stress - self.stress) * 0.05  # small contagion factor
        ...
        self.energy -= 0.12  # slightly faster drain

        # If low energy, try to move toward closest energy source
        ...
        # Energy depletion resets creature state
        if self.energy <= 0:
            self.energy = random.uniform(4, 7)
            self.stress = 0.0
            self.response = 1.0
...
def draw_grid(creatures, energy_sources):
    ...
    # Draw energy sources as yellow squares
    ...
    # Draw creatures as colored squares depending on species and mood
    for c in creatures:
...
def deserialize_creatures(json_str):
    ...
    creatures = []
    Creature._id_counter = 0  # Reset counter for new session
    for cdata in data:
        ...
        c.mood = cdata["mood"]
        # Manually set id & update counter to avoid duplicates
        c.id = cdata["id"]
...
# Initialize sim params in session_state (if not exist)
...
if "energy_sources" not in st.session_state:
    # Place randomly without overlapping creatures
    energy_sources = set()
    attempts = 0
...
with st.sidebar:
    ...
    st.session_state.sim_params['initial_creature_count'] = st.number_input("Initial Creature Count", 1, 50, st.session_state.sim_params['initial_creature_count'], 1)
    st.session_state.sim_params['energy_source_count'] = st.number_input("Energy Source Count", 0, 100, st.session_state.sim_params['energy_source_count'], 1)
    ...
    st.text_area("Creatures JSON", saved_creatures, height=180, key="save_creatures_area")
    st.text_area("Energy Sources JSON", saved_energy, height=80, key="save_energy_area")

    if st.button("Load Simulation from JSON"):
        try:
            loaded_creatures = deserialize_creatures(st.session_state.save_creatures_area)
            loaded_energy = deserialize_energy_sources(st.session_state.save_energy_area)
    ...
    if selected_creature:
        ...
        # Show charts of energy and stress history
        st.line_chart({"Energy": list(selected_creature.energy_history), "Stress": list(selected_creature.stress_history)})
...
# Show summary moods and stats under grid
st.markdown("### Creatures' Moods and Stats")
cols = st.columns(min(len(st.session_state.creatures), 10))
for idx, c in enumerate(st.session_state.creatures[:10]):
    with cols[idx]:
        st.write(f"ID {c.id}")
